...
export function HistoryModal({
  ...
  const formatarAcao = (acao: string) => {
    switch (acao) {
      ...
      case "entrada_atualizada":
        return "Entrada atualizada";
      case "status_alterado":
        return "Status alterado";
      case "progresso_alterado":
        return "Progresso atualizado";
      case "orcamento_detalhe_criado":
        return "Detalhes do orçamento criados";
      case "orcamento_status_alterado":
  ...
  const formatarNomeCampo = (campo: string): string => {
    const mapa: Record<string, string> = {
      ...
      placa: "Placa",
      finalNumeroQuadro: "Final do Quadro",
      descricao: "Descrição",
      observacoes: "Observações",
      enderecoEntrada: "Endereço (Entrada)",
      cepEntrada: "CEP (Entrada)",
      telefoneEntrada: "Telefone (Entrada)",
      frete: "Frete",
      dataOrcamento: "Data Orçamento",
  ...
  const formatarDetalhes = (item: HistoricoItem) => {
    ...
    if (acao === "status_alterado" || acao === "orcamento_status_alterado") {
      return (
        <span className="text-sm text-gray-500">
          De: <span className="font-medium">{detalhes.de}</span> para:{" "}
          <span className="font-medium">{detalhes.para}</span>
        </span>
      );
    }
    ...
    if (acao === "entrada_atualizada") {
      return (
        <div className="space-y-1 mt-1">
          {Object.entries(detalhes).map(([campo, valor]: [string, any]) => {
            if (
              valor &&
              typeof valor === "object" &&
              "antes" in valor &&
              ...
              return (
                <div key={campo} className="text-sm text-gray-600">
                  <span className="font-medium">
                    {formatarNomeCampo(campo)}:
                  </span>
                  <span className="ml-1 text-gray-500">
                    Antes:{" "}
                    <span className="font-medium">{valor.antes || "-"}</span>
                  </span>
                  <span className="mx-1">→</span>
                  <span className="text-gray-500">
                    Depois:{" "}
                    <span className="font-medium">{valor.depois || "-"}</span>
                  </span>
                </div>
              );
            }
